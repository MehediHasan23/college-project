...
const DataTable = () => {
    ...
    if (selectedRows.length > 0 && selectedOption) {
      ...
      if (batchUpdateArr.length > 0) {
        batchUpdateArr.forEach((row) => {
          fetch(`http://localhost:3001/assignTo/${row.id}`, {
            method: "PUT",
            headers: {
  ...
  useEffect(() => {
    (function () {
      fetch("http://localhost:3001/assignTo")
        .then((response) => {
    ...
    (async function () {
      const res = await (
        await fetch(`http://localhost:3001/assignUserList`)
      ).json();
      setNames(res);
    })();
  }, []);
  ...
  const handelSingleItemUpdate = (item) => {
    ...
    fetch(`http://localhost:3001/assignTo/${id}`, {
      method: "PUT",
      headers: {
        "Content-type": "application/json",
      },
